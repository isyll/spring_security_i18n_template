...
public class AuthController extends BaseController {
  ...
  public Mono<ResponseEntity<ApiResponse<User>>> signup(@RequestBody @Valid SignupRequest request) {
    return userService
        .registerUser(userMapper.toUser(request))
        .map(createdUser -> ok(createdUser, HttpStatus.CREATED));
  }

  @PostMapping("/refresh-token")
  @Operation(summary = "Refresh token", description = "Get new access token from refresh token.")
  public Mono<ResponseEntity<ApiResponse<JwtResponse>>> refreshToken(
      @RequestBody @Valid RefreshTokenRequest request) {
    String accessToken = authService.authenticateFromRefreshToken(request);
    return authService
        .authenticateFromRefreshToken(request)
        .map(accessToken -> ok(new JwtResponse(accessToken, request.getRefreshToken())));
  }
}
